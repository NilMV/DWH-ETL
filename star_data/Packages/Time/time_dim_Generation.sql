 ...
 SELECT   TO_NUMBER ( TO_CHAR ( mydate, 'yyyymmdd' ) ) AS date_key
  ...
  , 'Q'
    || TO_CHAR ( mydate, 'Q' )                                      AS quarter_desc
  ...
  , CASE
      WHEN mydate = ADD_MONTHS ( TRUNC ( mydate, 'Q' ), 3 ) - 1
      THEN 1
      ELSE 0
  ...
  , 'YR'
    || TO_CHAR ( mydate, 'yyyy' ) AS year_desc
  , 'YR'
    || TO_CHAR ( mydate, 'yy' )                                      AS year_sdesc
  ...
  , ADD_MONTHS ( TRUNC ( mydate, 'Y' ), 12 ) - TRUNC ( mydate, 'Y' ) AS days_in_year
     FROM
    (SELECT   TRUNC ( ADD_MONTHS ( SYSDATE, - 12 ), 'yy' ) - 1 + LEVEL AS mydate
         FROM dual
        CONNECT BY LEVEL <=
        (SELECT   TRUNC ( ADD_MONTHS ( SYSDATE, 24 ), 'yy' ) - TRUNC ( ADD_MONTHS ( SYSDATE, - 12 ), 'yy' )
             FROM DUAL
        )
    ) ;

CREATE sequence times_seq_1 start with 1 increment BY 1 nocache nocycle ;

   UPDATE dim_time SET time_id = times_seq_1.nextval ;

   SELECT MIN ( time_id ) FROM dim_time; --100

   SELECT MAX ( time_id ) FROM dim_time; --830

   DELETE FROM dim_time WHERE extract ( YEAR FROM date_time_start ) = '2017';
